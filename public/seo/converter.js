(function (window) {
  const JpgToPngConvertor = (() => {
    function convertor(imageFileBlob, options) {
      ...
      function updateDownloadLink(jpgFileName, pngBlob) {
        ...
        const pngFileName = jpgFileName.replace('png', 'jpeg');
  ...
  const imageFileEl = document.querySelector('[id="files"]');
  imageFileEl.addEventListener('change', (event) => {
    const jpgImageFileBlob = event.currentTarget.files[0];
    // Validate
    if (jpgImageFileBlob.type.match(/image\/png/i) !== null || jpgImageFileBlob.type.match(/image\/webp/i) != null) {
